...
/**************************************************************************
 * Function Name : ADC_Get_Value_Auto_Trigger
 *
 * Description	 : Reading ADC value after interrupt occurrence from external interrupt
 *
 * INPUTS		 : void
 *
 * OUTPUTS 		 : void
 ...
 * Return		 : void
 **************************************************************************/
void ADC_Get_Value_Auto_Trigger(void)
{
...
int main(void)
{
	/*Setting input pin and activating internal pull up for interrupting signal by a switch*/
	DIO_SetPinDirection(PinB2,INPUT);
	DIO_WritePin(PinB2,OUTPUT);
	...
	/*Initilizing interrupts*/
	interrupt_init();
	LCD_clearScreen();
	LCD_clearScreen();
	/*Setting callback function to get called from external interrupt from lower layer*/
	ADC_SetCallback_Auto_Trigger(ADC_Get_Value_Auto_Trigger);
#if (INTERRUPT_ENABLED==1)
